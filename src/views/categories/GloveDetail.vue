    ...
    <div class="container-detail">
        ...
        <div class="product-detail">
            <div class="img-container">
                <img class="detail-img" :src="'./img/' + gloves.gambar" />
            </div>
            <form class="detail-form" v-on:submit.prevent>
                <h3>{{ gloves.nama }}</h3>
                <h5>IDR {{ gloves.harga }}</h5>
...
export default {
    ...
    methods: {
        addChart() {
            if (this.pesanan.quantity) {
                this.pesanan.product = this.gloves
    ...
    async mounted() {
        const glove = await axios.get('http://localhost:3000/glove/' + this.$route.params.id)
        this.gloves = glove.data;
    }
}
</script>
