    ...
    <div class="container-detail">
        ...
        <div class="product-detail">
            <div class="img-container">
                <img class="detail-img" :src="'./img/' + sandals.gambar" />
            </div>
            <form class="detail-form" v-on:submit.prevent>
                <h3>{{ sandals.nama }}</h3>
                <h5>IDR {{ sandals.harga }}</h5>
...
export default {
    ...
    methods: {
        addChart() {
            if (this.pesanan.quantity) {
                this.pesanan.product = this.sandals
    ...
    async mounted() {
        const sandal = await axios.get('http://localhost:3000/sandal/' + this.$route.params.id)
        this.sandals = sandal.data;
    }
}
</script>
