    ...
    <div class="container-detail">
        ...
        <div class="product-detail">
            <div class="img-container">
                <img class="detail-img" :src="'./img/' + belts.gambar" />
            </div>
            <form class="detail-form" v-on:submit.prevent>
                <h3>{{ belts.nama }}</h3>
                <h5>IDR {{ belts.harga }}</h5>
...
export default {
    ...
    methods: {
        addChart() {
            if (this.pesanan.quantity) {
                this.pesanan.product = this.belts
    ...
    async mounted() {
        const belt = await axios.get('http://localhost:3000/belt/' + this.$route.params.id)
        this.belts = belt.data;
    }
}
</script>
