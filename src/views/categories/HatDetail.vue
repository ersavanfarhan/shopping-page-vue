    ...
    <div class="container-detail">
        ...
        <div class="product-detail">
            <div class="img-container">
                <img class="detail-img" :src="'./img/' + hats.gambar" />
            </div>
            <form class="detail-form" v-on:submit.prevent>
                <h3>{{ hats.nama }}</h3>
                <h5>IDR {{ hats.harga }}</h5>
...
export default {
    ...
    methods: {
        addChart() {
            if (this.pesanan.quantity) {
                this.pesanan.product = this.hats
    ...
    async mounted() {
        const hat = await axios.get('http://localhost:3000/hat/' + this.$route.params.id)
        this.hats = hat.data;
    }
}
</script>
