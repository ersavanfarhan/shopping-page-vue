    ...
    <div class="container-detail">
        ...
        <div class="product-detail">
            <div class="img-container">
                <img class="detail-img" :src="'./img/' + mountaineerings.gambar" />
            </div>
            <form class="detail-form" v-on:submit.prevent>
                <h3>{{ mountaineerings.nama }}</h3>
                <h5>IDR {{ mountaineerings.harga }}</h5>
...
export default {
    ...
    methods: {
        addChart() {
            if (this.pesanan.quantity) {
                this.pesanan.product = this.mountaineerings
    ...
    async mounted() {
        const mountaineering = await axios.get('http://localhost:3000/mountaineering/' + this.$route.params.id)
        this.mountaineerings = mountaineering.data;
    }
}
</script>
