    ...
    <div class="container-detail">
        ...
        <div class="product-detail">
            <div class="img-container">
                <img class="detail-img" :src="'./img/' + jackets.gambar" />
            </div>
            <form class="detail-form" v-on:submit.prevent>
                <h3>{{ jackets.nama }}</h3>
                <h5>IDR {{ jackets.harga }}</h5>
...
export default {
    ...
    methods: {
        addChart() {
            if (this.pesanan.quantity) {
                this.pesanan.product = this.jackets
    ...
    async mounted() {
        const jacket = await axios.get('http://localhost:3000/jacket/' + this.$route.params.id)
        this.jackets = jacket.data;
    }
}
</script>
